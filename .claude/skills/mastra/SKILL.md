...
Verified against `@mastra/core@1.60.0` (types + dist read directly, Aug 2026). `@mastra/memory` / `@mastra/pg` / `@mastra/libsql` / `@mastra/observability` claims are **not** type-verified — those packages are not installed in Colophon.
...
| `Processor` (type) · `ChunkType` (type) · `createScorer`                                                  | `@mastra/core/processors` · `@mastra/core/stream` (**not** `/processors`) · `@mastra/core/evals` |
| `SpanType`, `AnySpan`, `SpanOutputProcessor`, `TracingContext` · `Observability`, `MastraStorageExporter` | `@mastra/core/observability` · `@mastra/observability`                                           |
...
export const agent = new Agent({
	...
	hooks: {
		beforeToolCall: ({ toolName, input }) => {}, // return NOTHING to proceed; `{ proceed: false, output }` short-circuits
...
for await (const c of stream.fullStream) {
	if (c.type === 'tool-call-approval') {
		const next = await agent.approveToolCall({
			runId: stream.runId,
...
export const wf = createWorkflow({ id: 'wf', inputSchema, outputSchema, stateSchema })
	...
	.branch([
		[async ({ inputData }) => inputData.v > 10, stepA],
...
const stream = run.stream({ inputData, closeOnSuspend: false })   // NOT async, unlike agent.stream()
...
new Agent({
	...
	instructions: '…',
	model: openai('gpt-5'),
	skills: [codeReview, './skills/testing']
});
```
...
await parent.stream('...', {
	...
	delegation: {
		onDelegationStart: async (c) => ({
			proceed: true,
			modifiedPrompt: `${c.prompt}\nBe brief.`,
			modifiedMaxSteps: 5
		}),
		onDelegationComplete: async (c) =>
			c.error ? (c.bail(), { feedback: `failed` }) : { resultText: '...' },
...
import type { ChunkType } from '@mastra/core/stream'; // NOT re-exported from /processors
...
export class Guard implements Processor {
	...
	async processOutputStep({ text, abort, retryCount }) {
		if (!ok(text) && retryCount < 3)
			abort('Too vague', { retry: true, metadata: { kind: 'quality' } });
		return [];
	}
}
```
...
const sources = createScorer({
	...
	.analyze({
		...
		outputSchema: z.object({ hasSources: z.boolean() }),
...
Chain `.preprocess()` → `.analyze()` → `.generateScore()` (required) → `.generateReason()`. Each step is a function OR a prompt object (`{ description, createPrompt, judge? }`, plus `outputSchema` on `preprocess`/`analyze`); the `judge` LLM runs only for prompt objects. Prompt-mode `generateScore` takes **no** `outputSchema` and **no** `calculateScore` — the runtime pins the judge to `z.object({ score: z.number() })` and reads `.score`. (`calculateScore` appears in the shipped upstream docs but exists in neither the types nor the runtime.)
...
Export order: internal spans dropped → `excludeSpanTypes` → `spanOutputProcessors` → `spanFilter`. Child spans: `context?.tracingContext?.currentSpan?.createChildSpan({ type: SpanType.GENERIC, name, input })` then `span.end({ output })` / `span.error({ error })` — `type` is the `SpanType` **enum**, a bare `'generic'` string does not typecheck. `result.traceId` / `result.spanId` exist on both generate and stream.

**Stream chunks** — every chunk is `{ type, runId, from, payload }` (plus optional `metadata`), `from ∈ AGENT|USER|SYSTEM|WORKFLOW|NETWORK`. Use `stream.fullStream` (the 40+ member `ChunkType` union), not the streaming guide's 7-type list.
...
for await (const chunk of stream.fullStream) {
	...
	if (chunk.type === 'tripwire') {
		log(chunk.payload.processorId, chunk.payload.reason);
		break;
	}
}
```
...
- `.parallel()`/`.branch()` outputs are **keyed by step id**; the next step's `inputSchema` must model that object, with `.optional()` keys for branches. `.branch()` runs **every** arm whose condition is truthy, concurrently (`Promise.all`) — not just the first. The upstream docs claim "only one branch executes"; the engine disagrees. Make the conditions mutually exclusive yourself.
- Branch/loop conditions receive `inputData` (plus `iterationCount`, first eval = 1). The reference page's `({ context })` is stale docs.
- `.foreach()` default `concurrency` is **1**. The step inside gets one item; the step **after** gets the whole array. Chaining `.foreach().foreach()` stays **flat** (`T[]`, not `T[][]`) — the second foreach re-maps the first's result array element-wise. You only get nesting when the inner step itself returns an array.
...
- `setState` is typed `(state) => Promise<void>` — `await` it (the upstream Step reference page shows it sync). It **replaces** state, so always spread.
...
- `lastMessages: false` disables history **entirely** (nothing loaded _or saved_). For load-without-write use `readOnly: true`; for unlimited use `Number.MAX_SAFE_INTEGER`.
...
- `@mastra/core/observability` owns the tracing _types and enums_ — `SpanType`, `AnySpan`, `SpanOutputProcessor`, `TracingContext`, `SamplingStrategy`, the config interfaces. Only the runtime wiring (`Observability`, `MastraStorageExporter`) lives in `@mastra/observability`.
